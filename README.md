...
- `--interface-name <name>` / `mihomo.interface_name`: force temporary mihomo outbound dials through a network interface such as `en0`; use `auto` to resolve the current macOS default physical interface at each startup, or leave it empty to avoid forcing an interface.
