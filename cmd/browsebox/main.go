...
Flags:
  ...
  --interface-name name     Network interface for mihomo outbound dials; "auto" uses the current default physical interface
...
func newFlagSet(name string, opts *app.Options) *flag.FlagSet {
	...
	flags.StringVar(&opts.MihomoInterfaceName, "interface-name", opts.MihomoInterfaceName, "network interface for mihomo outbound dials; auto uses the current default physical interface")
