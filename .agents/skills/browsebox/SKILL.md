...
Do not use `--headless` for first login. Stop the visible session before reusing the same `--chrome-profile-dir` headlessly. Keep the same proxy node; when a fixed node is required, pass `--select-fastest=false` so local config cannot replace it.
