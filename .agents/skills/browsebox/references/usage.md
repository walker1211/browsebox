...
Stop the visible login session before reopening the profile headlessly:

```bash
./browsebox stop
./browsebox start \
  --headless \
  --select-fastest=false \
  --group "<group>" \
  --node "<node>" \
...
Through DevTools, verify the final page is not a login URL and that authenticated navigation or account controls are present. A stored Cookie alone is not sufficient evidence that the session works.
